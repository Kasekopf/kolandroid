...
public class InventoryFragment extends BaseGameFragment<Void, InventoryModel> implements OnCreateFragmentListener {
	...
	public void setup(Fragment f, String tag) {
		...
		fragment.setOnSelection(new OnListSelection<InvItem>() {
			@Override
			public boolean selectItem(InvItem item) {
				ItemDialog dialog = ItemDialog.create(item);
				dialog.setOnSelection(new OnListSelection<ActionItem>() {
					@Override
					public boolean selectItem(ActionItem item) {
						item.select(getModel());
						return true;
					}
				});
